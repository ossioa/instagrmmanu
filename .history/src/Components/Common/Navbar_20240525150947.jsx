...
import PostList from '../Post/PostList'; // Import PostList
    ...
    <>
      <nav className={`flex justify-between items-center px-4 py-2 bg-gray-100 sticky top-0 z-20 shadow sm:px-8 md:px-12 ${isOpen ? 'hidden md:flex' : 'flex'}`}>
        ...
        <div className='hidden md:flex justify-center flex-grow'>
          <Search searchQuery={searchQuery} setSearchQuery={setSearchQuery} /> {/* Pass search props */}
        </div>
      ...
      <PostList searchQuery={searchQuery} /> {/* Pass search query */}
    </>
  );
}
