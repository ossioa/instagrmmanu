...
import AvatarDisplay from '../Profile/AvatarDisplay'; // Importez AvatarDisplay
import { useAuth } from '../../contexts/AuthContext'; // Importez useAuth

const Navbar = () => {
  const [showCreatePost, setShowCreatePost] = useState(false);
  const [showAvatarUpload, setShowAvatarUpload] = useState(false); 
  const { currentUser } = useAuth(); // Obtenez l'utilisateur actuel à partir du contexte d'authentification
  ...
  return (
    <div className='flex justify-between items-center px-12 py-2 bg-gray-100 sticky top-0 z-20 shadow'>
      ...
      <Search />
      <div className='flex justify-between gap-x-4 items-cent'>
        <button
          onClick={() => setShowAvatarUpload(true)}
          className=' p-2 rounded-full hover:bg-blue-600 transition duration-300'
          title="Upload Avatar"
        >
