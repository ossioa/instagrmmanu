    ...
    <>
      ...
      {isOpen && (
        <div className='flex justify-between items-center w-full px-4 py-2 bg-gray-100 sticky top-0 z-20 shadow md:hidden'>
          <button
            onClick={() => setShowAvatarUpload(true)}
            className='bg-blue- text-white  rounded-full transition duration-300 w-10 h-10 md:w-16 md:h-16 lg:w-24 lg:h-24 text-xs md:text-sm lg:text-base'
            title="Upload Avatar"
          > 
            {currentUser && <AvatarDisplay userId={currentUser.uid} />} 
          </button>
          ...
          <button
            onClick={() => setShowCreatePost(true)}
            className='bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded transition duration-300 ease-in-out flex flex-col items-center gap-y-2 w-10 h-10 md:w-16 md:h-16 lg:w-24 lg:h-24 text-xs md:text-sm lg:text-base'
          >
            Create Post
          </button>
          <Modal isOpen={showCreatePost} closeModal={() => setShowCreatePost(false)}>
