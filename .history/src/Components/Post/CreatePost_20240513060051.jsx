...
import { storage, db } from './path/to/firebaseConfig';
...
const CreatePost = () => {
  ...
  const { user } = useAuth(); // Assuming the AuthContext provides an object { user }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const fileRef = ref(storage, `${user.uid}/posts/${Date.now()}_${file.name}`);
