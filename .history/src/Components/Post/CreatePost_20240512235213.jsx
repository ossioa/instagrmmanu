...
const CreatePost = () => {
  ...
  useEffect(() => {
    ...
    if (data && currentUser) {
      const { fileData, captionData } = JSON.parse(data);
      const blob = new Blob([fileData], { type: 'image/jpeg' }); // Assurez-vous que le type correspond
      const file = new File([blob], "upload.jpg"); // Nommer le fichier comme vous le souhaitez
      setFile(file);
      setCaption(captionData);
      sessionStorage.removeItem('postCreationData'); // Nettoyage
      handleSubmit(); // Essayer de soumettre après que les données soient prêtes
    }
  }, [currentUser]);
